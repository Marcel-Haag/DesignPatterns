...
// in two ways - for printing and for collecting elements
// in groups using dynamic-dispatch
namespace ObjectStructure
{
    ...
    class CountVisitor : IVisitor
    {
        ...
        {
            element.Accept(this);
            if (element.Link != null) CountElements(element.Link); if (element.Next != null) CountElements(element.Next);
        }

        // Elements with links are not counted public void Visit(ElementWithLink element) {
    ...
    class Client
    {
        static void Main()
        {
            ...
            new Element(
              new Element(
              new Element(null))))));
            Console.WriteLine("Count the Elements"); CountVisitor visitor = new CountVisitor();
